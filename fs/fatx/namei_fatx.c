 ...
 *  Modified for FATX support 2018 by Edward Humes <edward@thistlegame.net>
 */
...
/* Characters that are undesirable in a FATX file name */
			   ...
			   struct timespec64 *ts, struct fat_slot_info *sinfo)
{
	...
	memcpy(de.name, 0xFF, MSDOS_NAME)
...
{
	printk(KERN_DEBUG, "FATX filesystem support initialized!\n");
	return register_filesystem(&msdos_fs_type);
}

static void __exit exit_msdos_fs(void)
{
	printk(KERN_DEBUG "FATX filesystem support deinitialized!\n");
	unregister_filesystem(&msdos_fs_type);
}

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Edward Humes");
