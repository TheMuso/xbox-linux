 ...
 */
static struct inode *fat_dget(struct super_block *sb, __u32 i_logstart)
{
 ...
 */
static
struct inode *fat_rebuild_parent(struct super_block *sb, __u32 parent_logstart)
{
	__u32 search_clus, clus_to_match;
	struct msdos_dir_entry *de;
...
static struct dentry *fat_get_parent(struct dentry *child_dir)
{
	...
	if (!fat_get_dotdot_entry(d_inode(child_dir), &bh, &de)) {
		__u32 parent_logstart = fat_get_start(sbi, de);
